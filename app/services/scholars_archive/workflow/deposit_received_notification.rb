    ...
    class DepositReceivedNotification < ScholarsArchive::Workflow::AbstractNotification
      ...
      def message
        if SolrDocument.find(work_id)['resource_type_tesim']&.include?('Article')
          ...
          doi_message = (doi == "https://doi.org/10.7267/#{work_id}") ? "Your temporary DOI is https://doi.org/10.7267/#{work_id} . This DOI will not be live until the dataset is approved, but it won't change. If you supplied an existing DOI or declined to register one when you submitted this work, please disregard this temporary DOI.<br>" : ""
