class NestedOrderedTableOfContents < ActiveTriples::Resource
  ...
  property :tableofcontents, predicate: ::RDF::Vocab::DC.creator
  ...
  def initialize(uri=RDF::Node.new, parent=nil)
    if uri.try(:node?)
      uri = RDF::URI("#nested_ordered_tableofcontents#{uri.to_s.gsub('_:', '')}")
