class NestedGeoLocation < ActiveTriples::Resource
  ...
  def initialize(uri, parent=nil)
    if uri.try(:node?)
      uri = RDF::URI("#geo_location_#{uri.to_s.gsub('_:','')}")
  ...
  def self.set_nested_geo_locations(params)
    if params[:generic_work].nil?
      params
    else
      unless params[:generic_work][:nested_geo_location_attributes].nil?
        geo_locations = params[:generic_work][:nested_geo_location_attributes] || {}
        geo_locations.select{|k,v| v[:geonames_url].blank? == false && v[:id].nil? }.each_pair do |key, location|
          ...
          unless geonames_id.nil?
            ...
            params[:generic_work][:nested_geo_location_attributes][key][:geonames_url] = url
            # TODO: Implement or update TriplePoweredResource to retrieve
            # name given a geoname url
            # t = TriplePoweredResource.new(RDF::URI(url))
            # name = t.attributes['http://www.geonames.org/ontology#name'].first
            # params[:generic_work][:nested_geo_location_attributes][key][:name] = name
          end
