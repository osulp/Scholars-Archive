class NestedOrderedAltTitle < ActiveTriples::Resource
  ...
  property :alt_title, predicate: ::RDF::Vocab::DC.creator
  ...
  def initialize(uri=RDF::Node.new, parent=nil)
    if uri.try(:node?)
      uri = RDF::URI("#nested_ordered_alt_title#{uri.to_s.gsub('_:', '')}")
