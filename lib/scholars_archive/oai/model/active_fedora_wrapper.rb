class ScholarsArchive::OAI::Model::ActiveFedoraWrapper < ::OAI::Provider::Model
  include ScholarsArchive::OAI::Concern::ClassMethods
  ...
  def sets
    ...
    result.each do |col|
      test = ActiveFedora::SolrService.query("desc_metadata__primarySet_ssi: #{RSolr.escape('http://ir.library.oregonstate.edu/resource/'+ col['id'])}", :rows=>1)
  ...
  def build_query(selector, options={})
    ...
    if set
      query_pairs = "desc_metadata__set_sim: #{RSolr.escape('http://ir.library.oregonstate.edu/resource/' + set)}"
    elsif !selector.blank? && selector!= :all
      id = selector.split("/").last
      query_pairs = "id:#{RSolr.escape('ScholarsArchive:' + id)}"
    else
      query_pairs = "active_fedora_model_ssi:* -active_fedora_model_ssi:GenericCollection"
    end
  ...
  def build_wrapped(pseudo_obj, solr_doc)
    wrapped = ScholarsArchive::OAI::Model::SolrInstanceDecorator.new(pseudo_obj)
    #add set
    sets = []
