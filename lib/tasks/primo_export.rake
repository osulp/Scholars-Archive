...
namespace :scholars_archive do
  ...
  task :primo_export do
    ...
    require 'csv'
    docs = ActiveFedora::SolrService.query('has_model_ssim:* AND -has_model_ssim:FileSet AND -has_model_ssim:Hydra* AND -has_model_ssim:ActiveFedora* AND -has_model_ssim:AdminSet', {rows: 100000, fl: 'has_model_ssim,id,title_tesim'})
    CSV.open('/data0/hydra/shared/primo-recommender-ouput.csv', 'w') do |csv|
      csv <<  ['key','name','tags','description','image_url','url','url_text','internal_note']
      docs.each do |doc|
