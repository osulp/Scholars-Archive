...
namespace :scholars_archive do
  ...
  def set_conf_values
    ...
    collection_conf_name_mapping.each do |collection, conf_name|
      ...
      docs = ActiveFedora::SolrService.query(solr_query_str, rows: 100_000)
      ...
      docs.each do |doc|
        ...
        if doc['has_model_ssim'] && doc['has_model_ssim'].select(&:present?).count.positive? && %w[Article ConferenceProceedingsOrJournal].include?(doc['has_model_ssim'].first)
