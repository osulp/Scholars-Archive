...
namespace :scholars_archive do
  ...
  def cleanup_process(solr_query_str:, model_class:, required_flag:)
    data = ActiveFedora::SolrService.get(solr_query_str, rows: 100_000)

    # Create logger
    datetime_today = DateTime.now.strftime('%m-%d-%Y-%H-%M-%p') # "10-27-2017-12-59-PM"
    logger = ActiveSupport::Logger.new("#{Rails.root}/log/sa-cleanup-#{model_class}-#{datetime_today}.log")
    logger.info "Cleaning up degree_field and/or degree_grantors in #{model_class}"
    ...
    data['response']['docs'].each do |doc|
      ...
      next unless doc['has_model_ssim'] && doc['has_model_ssim'].select(&:present?).count.positive?
      ...
      begin
        ...
        if work.degree_field.present? && (!work.degree_field.start_with? 'http')
          ...
          if custom_degree_field_mapping[input_label]
            ...
            uri_from_label = degree_field_options.select { |label, _uri| label.split('-').first.strip.downcase.tr('.', '') == input_label_clean }.flatten.second
          end

          if uri_from_label
            logger.info "\t ready to update from \"#{work.degree_field}\" to #{uri_from_label}"
            new_attributes[:degree_field] = uri_from_label
          else
            logger.info "\t no match found for degree_field \"#{work.degree_field}\" in work with id #{doc['id']}"
          end
        end
        ...
        if work.degree_field.present? && (work.degree_field.start_with? 'http')
          logger.info "\t degree_field \"#{work.degree_field}\" is already a uri in work with id #{doc['id']}"
        end

        # log degree_field that are already uris
        if work.degree_grantors.present? && (work.degree_grantors.start_with? 'http')
          logger.info "\t degree_grantor \"#{work.degree_grantors}\" is already a uri in work with id #{doc['id']}"
        end
