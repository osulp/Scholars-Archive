namespace :scholars_archive do
  ...
  def retrieve_model_name
    ...
    input_list = ["5712m880q", "5999n672q", "5q47rs50r", "6395wb890", "9p290d94w", "fj236557c", "h702q7965", "jd473182z", "jw827h15c", "s4655j71h", "sj139549f", "tq57nv23h", "vm40xw219"]
    ...
    input_list.each do |id|
      begin
        ...
        Rails.logger.info "#{w.id}: #{work_model}"

      rescue => e
        Rails.logger.info "\tfailed to retrieve model name for work #{id}: #{e.message} #{e.backtrace}"
      end
    end
  end
    ...
    to_be_fixed.each do |id|
      ...
      begin
        ...
        if workflow_state.nil?
          ...
          Rails.logger.info "\t[no workflow state set] visitility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
          ...
          if w.embargo && w.embargo_release_date && w.embargo_release_date > DateTime.now
            ...
            if w.visibility_during_embargo && w.visibility_during_embargo == "authenticated" && w.visibility == "restricted"
              ...
              if w.save
                Rails.logger.info "\tsuccessfully changed visibility from restricted to #{w.visibility} for work #{w.id} (#{work_model})"
                counter += 1
              else
                Rails.logger.info "\t[unable to save work] visitiliby was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
              end
            else
              Rails.logger.info "\t[visibility_during_embargo not restricted or not set] visibility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
            end

            # fix visibility for child members if any
            fix_child_member_visibility_during_embargo(parent_work: w)
            ...
            if w.visibility_after_embargo && w.visibility_after_embargo == "authenticated" && w.visibility == "restricted"
              ...
              if w.save(validate: false)
                ...
                Rails.logger.info "\t[unable to save work] visibility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
              end
            else
              Rails.logger.info "\t[visibility_after_embargo not restricted or not set] visibility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
            end

            # fix visibility for child members if any
            fix_child_member_visibility_after_embargo(parent_work: w)
          else
            Rails.logger.info "\t[embargo and/or embargo_release_date not set] visibility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
          end
        else
          # workflow is known but not marked as 'deposited', log and find out what to do for this case
          Rails.logger.info "\t[workflow state set but not marked as 'Deposited'] visibility was not changed for work #{w.id} (#{work_model}), visibility_ssi: #{solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{workflow_state}\", embargo: {#{w.embargo.to_hash.to_s if w.embargo}}"
        end

      rescue => e
        Rails.logger.info "\tfailed to fix visibility for work #{id}: #{e.message}"
      end
    end
  ...
  def set_grad_year_if_nil(w)
    if ["undergraduate_thesis_or_project", "graduate_thesis_or_dissertation", "honors_college_thesis", "purchase_e_resource", "graduate_project"].include?(w.model_name.singular)
      if w.graduation_year.nil?
        w.graduation_year = ""
      end
    end
  end

  def fix_child_member_visibility_during_embargo(parent_work:)
    parent_work.ordered_members.to_a.each do |f|
      if f.embargo && f.visibility_during_embargo && f.visibility_during_embargo == "authenticated" && f.visibility == "restricted"
        Rails.logger.info "\tfixing visibility for child work #{f.id} - #{f.model_name.plural} (parent work #{parent_work.id}) so that it matches existing embargo visibility if any"
        child_solr_index = f.to_solr
        f.read_groups = ["registered"]
        if f.save
          Rails.logger.info "\tsuccessfully changed visibility from restricted to #{f.visibility} for child work #{f.id} - #{f.model_name.plural} (parent work #{parent_work.id})"
        else
          Rails.logger.info "\t[unable to save child work] visibility was not changed for child work #{f.id} #{f.model_name.plural} (parent work #{parent_work.id}), visibility_ssi: #{child_solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{child_solr_index['workflow_state_name_ssim']}\", embargo: {#{f.embargo.to_hash.to_s if f.embargo}}"
        end
      else
        Rails.logger.info "\t[visibility_during_embargo not restricted or not set] visibility was not changed for child work #{f.id} #{f.model_name.plural} (parent work #{parent_work.id}), visibility_ssi: #{child_solr_index['visibility_ssi']}, workflow_state_name_ssim: \"#{child_solr_index['workflow_state_name_ssim']}\", embargo: {#{f.embargo.to_hash.to_s if f.embargo}}"
      end
    end
  end

  def fix_child_member_visibility_after_embargo(parent_work:)
    parent_work.ordered_members.to_a.each do |f|
      if f.embargo && f.visibility_after_embargo && f.visibility_after_embargo == "authenticated" && f.visibility == "restricted"
        Rails.logger.info "\tfixing visibility for child work #{f.id} (parent work #{parent_work.id}) so that it matches existing embargo visibility if any"
        child_solr_index = f.to_solr
        f.read_groups = ["registered"]
