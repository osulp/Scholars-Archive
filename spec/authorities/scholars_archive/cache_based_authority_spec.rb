...
RSpec.describe ScholarsArchive::CacheBasedAuthority do
  ...
  describe '#terms' do
    before do
      allow(YAML).to receive(:safe_load).with(File.read(File.join(Rails.root, 'config/authorities.yml')), { permitted_classes: [Symbol], aliases: true }).and_return({ academic_units: { uri: 'blah', cache_expires_in_hours: 24, parser: 'Parsers::AcademicUnitsParser' } })
