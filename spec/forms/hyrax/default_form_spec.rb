...
RSpec.describe Hyrax::DefaultForm do
  let(:new_form) { described_class.new(Default.new, nil, instance_double('Controller')) }
  let(:user) do
  ...
  it 'has the proper secondary terms' do
    expect(new_form.secondary_terms).not_to include :license, :resource_type, :subject
  end
