...
describe '#log_formatting' do
  ...
  it 'should return log data into JSON format' do
  ...
  it 'should return log data and matches with the data it pass in' do
    logger.add(severity, msg1, progname) 
    log.rewind

    expect(JSON.parse(log.read)).to match({"date" => "#{Time.now}",
                                           "log_level" => "INFO",
                                           "req_id" => "",
                                           "req_uri" => "",
                                           "uri" => "",
  ...
  it 'will not parse correctly w/ some edge cases' do
    ...
    expect(JSON.parse(log.read)).not_to match({"date" => "#{Time.now}",
                                           "log_level" => "INFO",
                                           "req_id" => "123-456-789",
                                           "req_uri" => "/images/",
                                           "uri" => "",
