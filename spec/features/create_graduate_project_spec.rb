...
# Generated via
#  `rails generate hyrax:work Graduate Project`
require 'rails_helper'
require 'spec_helper'
include Warden::Test::Helpers

RSpec.describe GraduateProject, js: false do
  context 'when a logged in user' do
    ...
    before do
      ...
      login_as user
      visit new_hyrax_graduate_project_path
    end

    it 'renders the new form' do
      ordered_title_input = find(:css, 'input.nested-field.graduate_project_nested_ordered_title')
      fill_in ordered_title_input[:id], with: 'Test Graduate Project'
      ...
      select 'Zoology', from: 'graduate_project_degree_field'
      find('body').click
      ...
      expect(page).to have_content 'Add New Graduate Project'
    end

    context 'when sees default form values' do
      it 'default peerreviewed' do
        expect(page).to have_select('graduate_project_peerreviewed', selected: 'No')
      end
    end
  end
end
